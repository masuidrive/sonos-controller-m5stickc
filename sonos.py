...
class Sonos:
    ...
    def _issue_rendering_control_command(self, command, args=None):
        if args is None:
            args = [('InstanceID', 0), ('Channel', 'Master'), ('DesiredVolume', 10)]
        # Play/Pause/Next are all very similar.
        return upnp.send_command(
            self._base_url + '/MediaRenderer/RenderingControl/Control',
            'RenderingControl', 1, command, args
        )
    ...
    def volume(self, vol):
        response = self._issue_rendering_control_command('SetVolume',[('InstanceID', 0), ('Channel', 'Master'), ('DesiredVolume', vol)])
    def mute(self, flg):
        response = self._issue_rendering_control_command('SetMute',[('InstanceID', 0), ('Channel', 'Master'), ('DesiredMute', 1 if flg else 0)])
